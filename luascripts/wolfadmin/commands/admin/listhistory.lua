...
function commandListHistory(clientId, command, offset)
	...
	local CountHistory = db.getHistoryTotalCount()
			
	if not (CountHistory) then
		...
		local listFromHistory = db.listHistory(limit, offset)
		
		
		if (listFromHistory and #listFromHistory > 0) then
		
			for _, history in pairs(listFromHistory) do
					et.trap_SendConsoleCommand(et.EXEC_APPEND, "csay "..clientId.." \"^f Event ID: ^7"..string.format("%4s", history["id"]).."^f By: ^7"..string.format("%-20s", util.removeColors(db.getLastAlias(history["invoker_id"])["alias"])).." ^f Victim: ^7"..string.format("%-20s", util.removeColors(db.getLastAlias(history["victim_id"])["alias"])).." ^f Date: ^7"..os.date("%d/%m/%Y", history["datetime"]).."^f Action: ^7"..string.format("%-8s", history["type"].." ").."^f Reason or parameters: ^7"..history["reason"].."\";")
			end
